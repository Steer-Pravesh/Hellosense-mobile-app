  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 100 + (Platform.OS === 'web' ? 34 : insets.bottom), gap: 16, padding: 16 }} showsVerticalScrollIndicator={false}>
        ...
        {coaches.map((coach) => {
          const cHistory = sessionHistory.filter((s) => s.coachName === coach.name.replace('Coach ', ''));
          const cIncidents = cHistory.reduce((s, h) => s + h.safetyIncidents, 0);
          return (
            <View key={coach.id} style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
              ...
              <View style={styles.coachStats}>
                ...
                <CoachStatLine label="Response Rate" value="100%" colors={colors} />
              </View>
            </View>
          );
        })}
